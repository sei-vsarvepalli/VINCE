...
def _my_contact_group(user):
    admin_groups = VinceCommGroupAdmin.objects.filter(email__email=user.email, contact__active=True).values_list(
        "contact__id", flat=True
    )
    groups = user.groups.filter(
        groupcontact__contact__vendor_type__in=["Vendor", "Coordinator"], groupcontact__contact__in=admin_groups
    ).exclude(groupcontact__isnull=True)
    my_groups = []
    for ug in groups:
...
def _groupchat_case_participants(case):
    ...
    for participant in participants:
        if participant.participant:
            members.append(
                {
                    "value": participant.participant.vinceprofile.preferred_username,
                    "label": participant.participant.vinceprofile.preferred_username,
                }
            )
            ...
            if participant.group.user_set.count() > 0:
                try:
                    members.append(
                        {
                            "value": participant.group.groupcontact.contact.vendor_name,
                            "label": participant.group.groupcontact.contact.vendor_name,
                        }
                    )
                except:
                    continue
...
def _case_participants(case):
    ...
    posts = (
        Post.objects.filter(case=case)
        .exclude(author__vinceprofile__preferred_username__in=already_added)
        .distinct("author")
    )
    for post in posts:
        ...
        try:
            if _is_my_case(post.author, case.id):
                members.append(
                    {
                        "value": post.author.vinceprofile.preferred_username,
                        "label": post.author.vinceprofile.preferred_username,
                    }
                )
        except:
...
def autocomplete_vendor(request):
    if request.GET.get("term"):
        ...
        vendorlist = list(
            Group.objects.all()
            .exclude(groupcontact__isnull=True)
            .values_list("groupcontact__contact__vendor_name", flat=True)
        )
    else:
        vendorlist = list(
            Group.objects.all()
...
def autocomplete_coordinators(request, pk):
    ...
    if _is_my_case(request.user, pk):
        clist = (
            CaseMember.objects.filter(case=case, coordinator=True)
            .exclude(group__groupcontact__vincetrack=False)
            .values_list("group__groupcontact__contact__vendor_name", flat=True)
        )
...
def autocomplete_allvendors(request):
    ...
    for ug in groups:
        my_groups.append(
            {"euid": ed.encrypt(str(ug.groupcontact.contact.pk)), "vendor_name": ug.groupcontact.contact.vendor_name}
        )
...
def userapproverequest(request, *args, **kwargs):
    ...
    if kwargs and "caller" in kwargs and kwargs["caller"] == "vince":
        ...
        qs_admin = UserApproveRequest.objects.filter(
            contact__in=VinceCommGroupAdmin.objects.filter(
                email__email=request.user.username, contact__active=True
            ).values_list("contact__id", flat=True),
            status=UserApproveRequest.Status.UNKNOWN,
        )
    ...
    if request.method == "POST":
        if request.POST.get("pk"):
            ...
            if len(qupdate) == 1:
                ...
                logger.info(
                    f"Update of UserApproveRequest for {newuser} by {request.user.username} to new status {rec.status}. Full JSON result is {recjson}"
                )
    ...
    if request.method == "POST":
        if request.POST.get("tracker"):
            ...
            if request.POST.get("pk"):
                ...
                match = CaseTracking.objects.filter(
                    group__in=request.user.groups.all(), pk=request.POST.get("pk")
                ).first()
                if match:
                ...
                if not csm:
                    ...
                    return responder(err)
                newrec = CaseTracking(case=case, tracking=tracking, group=csm.group, added_by=request.user)
                try:
                    newrec.save()
                except Exception as e:
        ...
        if case:
            for cm in _my_groups_for_case(request.user, case):
                ...
                ctrec = {"group_id": cm.group.pk, "trackorg": cm.get_member_name()}
...
def autocomplete_users(request):
    if request.GET.get("term"):
        vendorlist = list(
            User.objects.filter(
                email__istartswith=request.GET.get("term"), vinceprofile__pending=False, is_active=True
            )
            .exclude(vinceprofile__service=True)
            .values_list("email", flat=True)
        )
    else:
        vendorlist = list(
            User.objects.filter(vinceprofile__pending=False, is_active=True)
            .exclude(vinceprofile__service=True)
            .values_list("email", flat=True)
        )
    data = json.dumps(vendorlist)
...
def create_action(title, user, case=None):
    ...
    action = VendorAction(title=title, user=user)
    if case:
        action.case = case
...
class TokenLogin(GetUserMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        if (
            ...
            and token_verify(self.request.POST["access_token"])
        ):
    ...
    def test_func(self):
        ...
        if a_type == "case":
            ...
            return UserThread.objects.filter(
                thread=message.thread.id, user=self.request.user
            ).exists() and PendingTestMixin.test_func(self)
        elif a_type == "track":
...
class DashboardView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        admin_group = (
            Group.objects.using("vincecomm")
            .filter(groupcontact__contact__vendor_name=settings.COGNITO_ADMIN_GROUP)
            .first()
        )
        if admin_group == None:
            messages.error(
        ...
        if is_in_group_vincetrack(self.request.user):
            ...
            form = CaseRoleForm()
            form.fields["owner"].choices = [(u.id, u.vinceprofile.vince_username) for u in assignable_users]

            context["form"] = form
            my_cases = _my_active_cases(self.request.user)
            cases = (
                my_cases.annotate(last_post_date=Max("post__created"))
            ...
            context["pending"] = VTCaseRequest.objects.filter(user=self.request.user, status=0).order_by(
                "-date_submitted"
            )
            return context
...
class RemoveFileView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if attachment.file:
            ...
            attachment.delete()
        messages.success(self.request, _("Your file was successfully removed."))
...
class VinceCommPrintReportsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["month"] = month
        context["newnotes"] = Case.objects.filter(
            note__datefirstpublished__year=year, note__datefirstpublished__month=month
        ).exclude(note__datefirstpublished__isnull=True)
        context["updated"] = Case.objects.filter(note__dateupdated__year=year, note__dateupdated__month=month).exclude(
            note__datefirstpublished__isnull=True
        )
        ...
        published_active_cases = active_cases.filter(
            Q(note__datefirstpublished__isnull=False) | Q(publicdate__isnull=False)
        )
        pre_public_active_cases = active_cases.exclude(
            Q(note__datefirstpublished__isnull=False) | Q(publicdate__isnull=False)
        )
        context.update(
            {
...
class CaseSummaryView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["limitedaccess"] = 1
        context["unread_msg_count"] = 0
        context["vendors"] = CaseMember.objects.filter(case__id=self.kwargs["pk"], coordinator=False).order_by(
            "group__name"
        )
        context["participants"] = CaseMember.objects.filter(
            case__id=self.kwargs["pk"], participant__isnull=False, coordinator=False
        )
...
class RequestAccessView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        case = get_object_or_404(Case, id=self.kwargs["pk"])
        new_track_ticket(
            "Case", "Case access requested", "User has requested access to this case", case, self.request.user.username
        )
...
class ChangeDefaultCaseAccess(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def test_func(self):
        ...
        if is_in_group_vincegroupadmin(self.request.user):
            ...
            self.admin = VinceCommGroupAdmin.objects.filter(
...
class UserCaseAccessView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["admin"] = VinceCommGroupAdmin.objects.filter(
            contact__id=gc.contact.id, email__email=self.request.user.email
        ).first()
        ...
        context["invited_users"] = VinceCommEmail.objects.filter(invited=True, contact=context["object"]).exclude(
            email__in=emaillist
        )
        context["eligible_users"] = VinceCommEmail.objects.filter(
            invited=False, contact=context["object"], email_list=False
        ).exclude(email__in=emaillist)
...
class CaseAccessView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def test_func(self):
        if is_in_group_vincegroupadmin(self.request.user):
            admin = VinceCommGroupAdmin.objects.filter(
                contact__id=self.kwargs.get("vendor_id"), email__email=self.request.user.email
            ).first()
            if admin:
                return PendingTestMixin.test_func(self)
        elif self.request.user.is_superuser:
    ...
    def get_context_data(self, **kwargs):
        ...
        context["access"] = list(
            CaseMemberUserAccess.objects.filter(user=context["u"], casemember__case__id__in=cases).values_list(
                "casemember__case__vuid", flat=True
            )
        )
        return context
        ...
        for case in all_cases:
            ...
            if case.vuid in cases:
                ...
                if access:
                    ...
                    if casemember:
                        access = CaseMemberUserAccess(user=user_id, admin=self.request.user, casemember=casemember)
                        access.save()
...
class PromoteUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if admin:
            admin.delete()
            action = VendorAction(
                title=f"{self.request.user.vinceprofile.vince_username} removed {user.vinceprofile.vince_username} as a group admin",
                user=self.request.user,
            )
            action.save()
            changes = ContactInfoChange(
                contact=gc.contact,
            ...
            if email:
                admin = VinceCommGroupAdmin(contact=gc.contact, email=email, comm_action=True)
                admin.save()
                ...
                action = VendorAction(
                    title=f"{self.request.user.vinceprofile.vince_username} made {user.vinceprofile.vince_username} a group admin",
                    user=self.request.user,
                )
                action.save()
                changes = ContactInfoChange(
...
class AdminView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def test_func(self):
        ga_groups = VinceCommGroupAdmin.objects.filter(
            email__email=self.request.user.email,
            contact__vendor_type__in=["Coordinator", "Vendor"],
            contact__active=True,
        )
        if len(ga_groups) > 0:
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("vendor_id"):
            ...
            if admin:
                ...
                context["notification_emails"] = VinceCommEmail.objects.filter(
                    contact=context["object"], email_list=True
                )
                if context["users"]:
                    emaillist = context["users"].values_list("username", flat=True)
                    context["invited_users"] = VinceCommEmail.objects.filter(
                        invited=True, contact=context["object"]
                    ).exclude(email__in=emaillist)
                    context["eligible_users"] = VinceCommEmail.objects.filter(
                        invited=False, contact=context["object"], email_list=False, status=True
                    ).exclude(email__in=emaillist)
                    context["groupcontact"] = GroupContact.objects.filter(
                        contact__id=self.kwargs.get("vendor_id")
                    ).first()
                    cases = _cases_for_group(Group.objects.filter(groupcontact=context["groupcontact"]).first())
                    context["caseaccess"] = CaseMemberUserAccess.objects.filter(casemember__case__in=cases)
                    # get service account
                    context["service"] = User.objects.filter(
                        groups__in=[context["groupcontact"].group], vinceprofile__service=True
                    ).first()
            ...
            ga_group = VinceCommGroupAdmin.objects.filter(
                email__email=self.request.user.username,
                contact__vendor_type__in=["Coordinator", "Vendor"],
                contact__active=True,
            ).first()
            if ga_group:
...
class AdminRemoveUser(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def test_func(self):
        if is_in_group_vincegroupadmin(self.request.user) and PendingTestMixin.test_func(self):
            vendor_id = self.kwargs.get("vendor_id")
            admin = VinceCommGroupAdmin.objects.filter(
                contact__id=vendor_id, email__email=self.request.user.email
            ).first()
            if admin:
                return True
        return False
        ...
        if user_type == "user":
            ...
            if user:
                ...
                if emuser:
                    ...
                    logger.debug(
                        f"Checking if {emuser.email} is group admin for {emuser.contact.vendor_name} or {my_group.vendor_name}"
                    )
                    admin = VinceCommGroupAdmin.objects.filter(email=emuser, contact=my_group).first()
                    if admin:
                        logger.warning(
                            f"Removing Group admin privileges for {emuser.email} from {emuser.contact.vendor_name}"
                        )
                        admin.delete()
                    ...
                    emuser.delete()
                    create_action(
                        f"{self.request.user.vinceprofile.vince_username} removed user {user.vinceprofile.vince_username}",
                        self.request.user,
                    )
                    # is this user a groupadmin of any other vendors, if not remove groupadmin group privs

                    messages.success(self.request, _("The user has been removed from your group"))
                    return redirect("vinny:admin")
            messages.error(self.request, _("Error: User was not found"))
            return redirect("vinny:admin")
            ...
            if user:
                if user.contact == my_group:
                    ...
                    create_action(
                        f"{self.request.user.vinceprofile.vince_username} removed user {user.email}", self.request.user
                    )
                    user.delete()
                    messages.success(self.request, _("The user has been removed from your group"))
                    return redirect("vinny:admin")
...
class AdminAddUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Request for user {username} to be added to {my_group.vendor_name} being processed by {self.request.user.username}"
        )
        ...
        if old_email:
            if old_email.email_list:
                return JsonResponse(
                    {
                        "response": "This email has already been added as notification-only email and cannot be used for user access."
                    },
                    status=200,
                )
                ...
                create_action(
                    f"{self.request.user.vinceprofile.vince_username} invited new user {users}", self.request.user
                )
                return JsonResponse({"response": "success"}, status=200)

        new_email = VinceCommEmail(
            email=users,
            name=username,
            email_type="Work",
            email_function="TO",
...
class ModifyEmailNotifications(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if email.email_function in ["TO", "CC"]:
            ...
            email.save()
            change = contact_update.create_contact_change(
                email.contact, email.email, "email notifications", "Enabled", "Disabled", self.request.user
            )
        else:
            email.email_function = "TO"
            email.save()
            change = contact_update.create_contact_change(
                email.contact, email.email, "email notifications", "Disabled", "Enabled", self.request.user
            )

        contact_update.send_ticket([change], email.contact, self.request.user)
        messages.success(self.request, "Got it!  Your preferences have been saved!")
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("type") == "user":
            ...
            context["email"] = VinceCommEmail.objects.filter(
                email=context["user"].email, contact=self.kwargs.get("vendor_id")
            ).first()
        ...
        context["post_url"] = reverse(
            "vinny:changeemail", args=[self.kwargs.get("vendor_id"), "email", context["email"].id]
        )
        return context
...
class MultipleGroupAdminView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["unread_msg_count"] = 0
        context["groups"] = VinceCommGroupAdmin.objects.filter(
            email__email=self.request.user, contact__vendor_type__in=["Coordinator", "Vendor"], contact__active=True
        )
...
class SearchThreadsView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if keyword:
            res = res.filter(
                Q(messages__content__icontains=self.request.POST["keyword"])
                | Q(subject__icontains=self.request.POST["keyword"])
            ).distinct()
        ...
        return render(
            ...
            {
                "threads": paginator.page(page),
                "empty_msg": "No threads match that filter",
                "page_class": "search_notes asyncpage",
            },
        )
...
class LimitedAccessSearch(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(
            request, self.template_name, {"cases": paginator.page(page), "total": res.count(), "limitedaccess": 1}
        )
...
class DashboardCaseView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("keyword", None):
            ...
            # search posts
            post_result = (
                PostRevision.objects.filter(post__case__in=my_cases)
                .extra(where=["search_vector @@ (to_tsquery('english', %s))=true"], params=[wordSearch])
                .values_list("post__case", flat=True)
            )
            if post_result:
...
class GroupChatView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.CreateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"user": self.request.user, "privgroupchat": True})
        return kwargs
...
class SendMessageUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("user_id"):
            ...
            context["user_tags"] = [
                x.email.email
                for x in VinceCommGroupAdmin.objects.filter(contact__vendor_id=self.kwargs.get("admin_id"))
            ]
        else:
            context["action"] = reverse("vinny:sendmsguser")
    ...
    def get_initial(self):
        ...
        if case == None:
            if user_id:
                ...
                case = [("", "--------")] + [
                    (q.id, q.get_title)
                    for q in _cases_for_group(Group.objects.filter(groupcontact__contact__vendor_id=group_id).first())
                ]
            else:
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"user": self.request.user, "privgroupchat": False})
        return kwargs
...
class SendMessageView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        if ticket and hasattr(ticket, "error"):
            ...
            messages.error(self.request, _(f"Error: {msg}"))
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            if self.kwargs.get("case"):
                members = list(
                    CaseMember.objects.filter(case__id=self.kwargs.get("case"), coordinator=True)
                    .exclude(group__groupcontact__vincetrack=False)
                    .values_list("group__groupcontact__contact__vendor_name", flat=True)
                )
...
class ReportDocumentCreateView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"report": get_object_or_404(VTCaseRequest, id=self.kwargs["pk"]), "user": self.request.user})
        return kwargs

    def form_valid(self, form):
        doc = form.save()
        messages.success(self.request, _("Your file was successfully uploaded."))
        vince_comm_send_sqs(
            "NewFile",
            "CaseRequestArtifact",
            self.report.vrf_id,
...
class CaseDocumentCreateView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"case": get_object_or_404(Case, id=self.kwargs["pk"]), "user": self.request.user})
        return kwargs
    ...
    def form_valid(self, form):
        ...
        vince_comm_send_sqs(
            "NewFile", "CaseArtifact", self.case.vuid, self.request.user.username, None, "Vendor Uploaded File"
        )
        return redirect("vinny:case", self.kwargs["pk"])
...
class UserCardView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.DetailView):
    ...
    def get_object(self):
        ...
        if len(euid) % 32:
            logger.info(
                "Invalid encrypted EUID value found returning user's self profile %s length does not match" % (euid)
            )
        ...
        try:
            ...
            logger.info(
                "Invalid encrypted EUID value found returning user's self profile %s error %s" % (euid, str(e))
            )
            return VinceProfile.objects.get(user=self.request.user)
...
class GroupCardView(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.DetailView):
    ...
    def get_object(self):
        ...
        if len(egid) % 32:
            logger.info(
                "Invalid encrypted EGID value found returning user's self profile %s length does not match" % (egid)
            )
            return None
        try:
            ed = ED(base64.b64encode(settings.SECRET_KEY.encode()))
            ...
            logger.info(
                "Invalid encrypted EGID value found returning user's self profile %s error %s" % (egid, str(e))
            )
            return None
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("case") and self.get_object() and self.get_object().default_access == False:
            ...
            admins = list(
                VinceCommGroupAdmin.objects.filter(contact=groupcontact.contact.id).values_list(
                    "email__email", flat=True
                )
            )
            casemember = CaseMember.objects.filter(case=self.kwargs.get("case"), group=groupcontact.group.id).first()
            emails = list(
                CaseMemberUserAccess.objects.filter(casemember=casemember).values_list("user__email", flat=True)
            )
...
class CaseView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
        ...
        if Case.objects.filter(id=self.kwargs["pk"]):
            if self.kwargs.get("vendor"):
                if CaseMember.objects.filter(id=self.kwargs["vendor"]):
                    return (
                        _is_my_case(self.request.user, self.kwargs["pk"])
                        and PendingTestMixin.test_func(self)
    ...
    def handle_no_permission(self):
        ...
        ctx = {"VINCECOMM_BASE_TEMPLATE": settings.VINCECOMM_BASE_TEMPLATE, "user": self.request.user}
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.session.get("preexistingcomment"):
            logger.debug(
                f'self.request.session.get("preexistingcomment") yields results: {self.request.session.get("preexistingcomment")}'
            )
        ...
        context["participants"] = CaseMember.objects.filter(
            case=case, participant__isnull=False, coordinator=False
        ) | CaseMember.objects.filter(case=case, coordinator=False, reporter_group=True).order_by("added")
        ...
        for member in members:
            if member.seen == False:
                create_action(
                    f"{member.group.groupcontact.contact.vendor_name} viewed case {case.vu_vuid}",
                    self.request.user,
                    case,
                )
                vince_comm_send_sqs(
        ...
        if lv:
            ...
            context["unseen_posts"] = (
                Post.objects.filter(case=case, created__gte=lv.date_viewed).exclude(author=self.request.user).count()
            )
        ...
        try:
            cviewed, created = CaseViewed.objects.update_or_create(
                case=case, user=self.request.user, defaults={"date_viewed": timezone.now}
            )
        except:
        ...
        logger.debug(f"the context returned for vincecomm CaseView is {context}")
        return context
...
def create_vendor_status_change(field, old_value, new_value, user, member, cv=None, case=None):
    if cv:
        action = VendorAction(
            title=f"{user.vinceprofile.vince_username} modified {field} for {cv.vul}", user=user, member=member
        )
        action.case = cv.case
    elif case:
        action = VendorAction(
            title=f"{user.vinceprofile.vince_username} modified {field} for {case.vu_vuid}", user=user, member=member
        )
        action.case = case

    action.save()

    change = VendorStatusChange(field=field, old_value=old_value, new_value=new_value, action=action)
    if cv:
        change.vul = cv
...
def update_status(member, request, vulid=None, affected=[], unknown=[], unaffected=[]):
    ...
    if affected:
        for vul in affected:
            ...
            status, created = CaseMemberStatus.objects.update_or_create(
                member=member, vulnerability=cv, defaults={"status": 1, "user": request.user, "approved": False}
            )
    if unknown:
        for vul in unknown:
            cv = CaseVulnerability.objects.filter(id=int(vul)).first()
            ...
            status, created = CaseMemberStatus.objects.update_or_create(
                member=member, vulnerability=cv, defaults={"status": 3, "user": request.user, "approved": False}
            )
    if unaffected:
        for vul in unaffected:
            ...
            status, created = CaseMemberStatus.objects.update_or_create(
                member=member, vulnerability=cv, defaults={"status": 2, "user": request.user, "approved": False}
            )
    ...
    vince_comm_send_sqs(
        ...
        member.group.name,
        f"Vendor {member.group.groupcontact.contact.vendor_name} updated Statement",
    )
...
class ViewStatusView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def test_func(self):
        ...
        return self.request.user.is_superuser or (
            _is_my_case(self.request.user, self.kwargs["pk"])
            and _my_group_for_case(self.request.user, case)
            and PendingTestMixin.test_func(self)
        )
    ...
    def form_valid(self, form):
        ...
        if old_stmt:
            ...
            if old_stmt.share != form.cleaned_data["share"]:
                create_vendor_status_change(
                    "share toggle", old_stmt.share, form.cleaned_data["share"], self.request.user, member, case=case
                )

        stmt = CaseStatement.objects.update_or_create(
            case=case,
            member=member,
            defaults={
        ...
        messages.success(
            self.request, _("Got it! Your status has been recorded and is pending approval by the coordination team.")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if contact_users:
            ...
            actions = (
                VendorAction.objects.filter(case=case)
                .filter(Q(user__in=contact_users) | Q(member=member))
    ...
    def form_valid(self, form):
        ...
        if status:
            if status.statement != form.cleaned_data["statement"]:
                create_vendor_status_change(
                    "statement", status.statement, form.cleaned_data["statement"], self.request.user, member, cv=vul
                )
            if status.references != form.cleaned_data["references"]:
                create_vendor_status_change(
                    "references", status.references, form.cleaned_data["references"], self.request.user, member, cv=vul
                )
        ...
        if form.is_valid():
            if self.request.POST.get("reply_to"):
                ...
                if find_post:
                    ...
                    post = ThreadedPost(case=case, parent=find_post, author=self.request.user)
                    post.save()
                else:
                    post = Post(case=case, author=self.request.user)
                    post.save()
            else:
                post = Post(case=case, author=self.request.user)
                post.save()
            ...
            vince_comm_send_sqs(
                ...
                self.request.user.username,
                str(post.id),
                "New Post from " + self.request.user.username,
            )
            # emails = send_usermention_notification(post, form.cleaned_data['content'])
            # send_post_email(post, emails)
            data = {"auto_members": _case_participants(post.case), "num_posts": self.get_queryset().count()}
...
class DeletePostView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        vince_comm_send_sqs(
            ...
            self.request.user.username,
            str(post.id),
            "Post removed by " + self.request.user.username,
        )
...
class PostDiffView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        diff = differ.compare(baseText.splitlines(keepends=True), newText.splitlines(keepends=True))
...
class EditContactView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    PostalFormSet = inlineformset_factory(
        VinceCommContact, VinceCommPostal, form=VCPostalForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    PhoneFormSet = inlineformset_factory(
        VinceCommContact, VinceCommPhone, form=VCPhoneForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    WebFormSet = inlineformset_factory(
        VinceCommContact, VinceCommWebsite, form=VCWebsiteForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    PgPFormSet = inlineformset_factory(
        VinceCommContact, VinceCommPgP, form=VCPgPForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    EmailFormSet = inlineformset_factory(
        VinceCommContact, VinceCommEmail, form=VCEmailForm, max_num=30, min_num=1, can_delete=True, extra=0
    )
    ...
    def form_invalid(self, form):
        ...
        forms = {
            "postal_formset": self.PostalFormSet(prefix="postal", queryset=postal, instance=self.contact),
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.debug(
            f" {self.__class__.__name__} context data Contact: {self.contact} and Contact id: {self.contact.id} and Vendor id: {self.contact.vendor_id}"
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.contact.version != sversion:
            ...
            messages.error(self.request, _(error_str))
                    ...
                    if cd["DELETE"]:
                        ...
                        if vince_user:
                            messages.error(
                                self.request,
                                _(
                                    'The email you are attempting to remove is associated with a VINCE user account. The group administrator can remove this user on the <a href="'
                                    + reverse("vinny:admin")
                                    + '">User Management</a> page.'
                                ),
                            )
                        ...
                        if oe:
                            messages.error(
                                self.request,
                                _(
                                    'The email you are attempting to add is already associated with this contact. Personal emails should be modified by a group admin on the <a href="'
                                    + reverse("vinny:admin")
                                    + '">User Management</a> page.'
                                ),
                            )
        ...
        for f in phoneformset:
            if f.is_valid():
                ...
                if old:
                    nochanges = all(
                        [
                            old.country_code == cd["country_code"],
                            old.phone == cd["phone"],
                ...
                for x in phoneformset.errors:
                    for k, v in x.items():
                        if "This field is required." not in v:
                            messages.error(self.request, f"Phone Number Validation Error: {k}: {v}")
            ...
            if f.is_valid():
                ...
                if old:
                    ...
                    if cd["DELETE"]:
                        ...
                        if cd:
                            obj, created = VinceCommPgP.objects.update_or_create(
                                id=old.id,
                                defaults={
                            ...
                            messages.error(self.request, _("PGP key not added: Invalid PGP Key Data"))
                else:
                    cd = contact_update.extract_pgp_info(cd)
                    if cd:
                        obj = VinceCommPgP(
                ...
                for x in pgpformset.errors:
                    for k, v in x.items():
                        if "This field is required." not in v:
                            if "Either PGP Key or ID is required" in v and not (
                                f.cleaned_data.get("pgp_key_data") or f.cleaned_data.get("pgp_key_id")
                            ):
                                continue

                            messages.error(self.request, f"PGP Key Validation Error: {v}")
                logger.debug(f"Error in {self.__class__.__name__} for phone formset, Error is {f.errors}")
        ...
        for f in webformset:
            if f.is_valid():
                ...
                if old:
                    nochanges = all(
                        [old.url == cd["url"], old.description == cd["description"], old.public == cd["public"]]
                    )
                    if nochanges and not (cd["DELETE"]):
                        continue
                    if cd["DELETE"]:
        ...
        if changes:
            ...
            if len(tm) == 0:
                messages.success(self.request, "Got it! Thanks for updating your contact information.")
...
class GenerateNewRandomColor(LoginRequiredMixin, TokenMixin, PendingTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.success(self.request, "Hope you like your new color!")
...
class CRView(LoginRequiredMixin, TokenMixin, PendingTestMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.debug(
            f"vinceutils.deepGet(context['report'].metadata,'ai_ml_system') is {vinceutils.deepGet(context['report'].metadata,'ai_ml_system')}"
        )
        ...
        # Mark coordinators for anonymous display to submitters BEFORE case creation
        # Only anonymize Track coordinators, not regular users/submitters
        # After case creation (when report.case exists), show real names
        if context["report"]:
            # Determine if we should anonymize: only before case creation
            should_anonymize = not hasattr(context["report"], "case")

            # Get followups and mark which are from coordinators
            followups = list(context["report"].crfollowup_set.all())
            for followup in followups:
                # Mark as coordinator only if anonymizing and user is a track coordinator
                followup.is_coordinator = should_anonymize and followup.user and is_in_group_vincetrack(followup.user)
            context["followups"] = followups

            # Mark attachment actions from coordinators
            for attachment in context["attachments"]:
                if attachment.action and attachment.action.user:
                    # Mark as coordinator only if anonymizing and user is a track coordinator
                    attachment.action.is_coordinator = should_anonymize and is_in_group_vincetrack(attachment.action.user)
                else:
                    attachment.action.is_coordinator = False
...
class UpdateReportView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        cr.save()
        messages.success(self.request, _("Your comment has been recorded."))
        vince_comm_send_sqs(
            "CRUpdate", "Ticket", self.report.vrf_id, self.request.user.username, str(cr.id), "New Comment on CR"
        )
...
class AdminReportsView(LoginRequiredMixin, TokenMixin, PendingTestMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["object_list"] = sorted(all_reports, key=lambda instance: instance.id, reverse=True)
        return context
...
class ReportView(LoginRequiredMixin, TokenMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        # create crfollowup
        cr = CRFollowUp(cr=newrequest, title="submitted vulnerability reporting form", user=self.request.user)
        cr.save()
        ...
        if reporter_email:
            ...
            try:
                ...
                logger.debug(
                    f"Error sending email to {reporter_email} in {self.__class__.__name__}, Error is {e.response['Error']['Message']}"
                )
                send_sns(vrf_id, "Sending ack email for vul reporting form", e.response["Error"]["Message"])
            except Exception as e:
                logger.debug(
                    f"Error sending email to {reporter_email} in {self.__class__.__name__} not ClientError, error is {e}"
                )
...
class CommVulReportAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # log the report
        logger.debug(f"Authenticated user submitted a report via API with the following info: {context}")
...
class CaseAPIView(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    permission_classes = (
        IsAuthenticated,
        PendingUserPermission,
        LimitedCaseAccessPermission,
    )
    lookup_field = "vuid"
            ...
            if vul:
                ...
                if _is_my_case(self.request.user, vul.case.id):
                    ...
                    if vendors:
                        ...
                        return Response(
                            {"vulnerability": vuln.data, "note": "NOT Public", "vendors": vv.data, "case": case.data}
                        )
                    else:
                        return Response(
                            {"vulnerability": vuln.data, "note": "NOT Public", "vendors": [], "case": case.data}
                        )
...
class UpdateVendorStatusAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
            ...
            if status_serializer.is_valid():
                        ...
                        return JsonResponse(
                            {
                                ...
                                "detail": "User belongs to multiple groups. Use vendor information API to retrieve vendor ID for organization",
                            },
                            status=rest_status.HTTP_400_BAD_REQUEST,
                        )
                else:
                    ...
                    return JsonResponse(
                        {
                            "type": "invalid-status",
                            "status": rest_status.HTTP_400_BAD_REQUEST,
                            "title": f'{status_serializer.data["status"]} is an invalid status',
                            "detail": "Affected/Not Affected/Unknown are valid statuses",
                        },
                        status=rest_status.HTTP_400_BAD_REQUEST,
                    )
                if re.match("cve-", status_serializer.data["vulnerability"], re.I):
                    ...
                    if not (vul_status):
                        return JsonResponse(
                            {
                                "type": "invalid-vulnerability",
                                "status": rest_status.HTTP_400_BAD_REQUEST,
                                "title": f'{status_serializer.data["vulnerability"]} is an invalid vulnerability',
                                "detail": "Use vulnerability API to find vulnerability name or CVE ID",
                            },
                            status=rest_status.HTTP_400_BAD_REQUEST,
                        )
                ...
                errors = {
                    "type": "invalid-format",
                    "detail": status_serializer.errors,
                    "status": rest_status.HTTP_400_BAD_REQUEST,
                    "title": "missing required fields or invalid format",
                }
            ...
            if len(vuls) == 1:
                ...
                if old_stmt:
                    ...
                    if old_stmt.references != references:
                        create_vendor_status_change(
                            "references", old_stmt.references, references, self.request.user, my_org, case=case
                        )
                    if old_stmt.share != status_serializer.data["share"]:
                        create_vendor_status_change(
                            "share toggle",
                            old_stmt.share,
                            status_serializer.data["share"],
                            self.request.user,
                            my_org,
                            case=case,
                        )
                stmt = CaseStatement.objects.update_or_create(
                    ...
                    member=my_org,
                    defaults={
                        "statement": status_serializer.data["statement"],
                ...
                if status:
                    ...
                    if status.references != references:
                        create_vendor_status_change(
                            "references", status.references, references, self.request.user, my_org, cv=vul_status
                        )

                status = CaseMemberStatus.objects.update_or_create(
        ...
        return JsonResponse(
            {"response": "success", "message": "Your status has been successfully recorded."}, status=200
        )
...
class VendorLookupView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("tag"):
            ...
            contacts = VinceCommContact.objects.filter(vendor_name=self.request.POST["tag"]).values_list(
                "groupcontact__group__id", flat=True
            )
            if contacts:
...
class CreateServiceAccountView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if old_user:
                return JsonResponse(
                    {"response": "This user may already exist.  Please check email and try again."}, status=200
                )
            ...
            messages.success(self.request, _("The account was successfully created."))
            ...
            vcemail, created = VinceCommEmail.objects.update_or_create(
                contact=gc.contact,
                email=form.cleaned_data["email"],
                defaults={"name": form.cleaned_data["preferred_username"], "status": True, "email_list": True},
            )
            ...
            action = VendorAction(
                title=f"{self.request.user.vinceprofile.vince_username} created service account {service.email}",
                user=self.request.user,
            )
            action.save()
            changes = ContactInfoChange(
            ...
            group_users = User.objects.filter(
                groups__in=[gc.group], is_active=True, vinceprofile__pending=False
            ).exclude(vinceprofile__service=True)
            for u in group_users:
                send_courtesy_email("service_account_created", u)

            return JsonResponse(
                {"response": "success", "action": reverse("cogauth:account_activation_sent")}, status=200
            )
...
class UnreadCountAjax(LoginRequiredMixin, TokenMixin, generic.TemplateView):
    ...
    def get(self, request, format=None):
        return JsonResponse({"unread": _unread_msg_count(self.request.user), "timestamp": datetime.now().timestamp()})
...
class CaseCSAFAPIView(generics.RetrieveAPIView):
    ...
    def handle_no_permission(self):
        ...
        return JsonResponse(
            {"error": "Report/Case identified either does not exist or you do not have permissions to view it"}
        )
